...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        pat_input = re.compile(r"model\.transformer\.blocks\.[0-9]+\.(ff_in|ff_up|up_proj|gate_proj)$")

        self.hooks = []

        for name, module in self.model.named_modules():
            if pat_input.search(name) and isinstance(module, nn.Linear):
                hook_in = OptimizedSparsityJaccardHook(
                    self.sparsity_rule, self.sparsity_param, layer_name=f"module.{name}", role="input"
                )
                handle_in = module.register_forward_pre_hook(hook_in.pre_forward_hook)
                self.hooks.append((hook_in, handle_in))
