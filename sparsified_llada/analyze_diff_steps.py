...
def layer_num_from_name(layer_name: str) -> int:
    """Extract numeric transformer block index from layer_name."""
    m = re.search(r"(?:blocks|layers|transformer\.blocks)\.(\d+)", layer_name)
    return int(m.group(1)) if m else 10**9

def find_file(directory: str, pattern: str) -> str:
    """Search for a file in the directory based on a glob pattern."""
    files = glob.glob(os.path.join(directory, pattern))
    if not files:
        raise FileNotFoundError(f"Could not find file with pattern '{pattern}' in {directory}")
    if len(files) > 1:
        print(f"[info] Multiple files found for '{pattern}', using: {files[0]}")
    return files[0]

def load_sparsity_json(path: str) -> dict:
...
def sparsity_json_to_df(data: dict) -> pd.DataFrame:
    ...
    for layer_name, steps in layers_dict.items():
        ln = layer_num_from_name(layer_name)
        for step_str, s in steps.items():
            ...
            rows.append({
                ...
                "count": int(s["count"]),
                "total_zeros": int(s.get("total_zeros", 0)),
                "total_elements": int(s.get("total_elements", 0)),
            })
...
def main():
    parser = argparse.ArgumentParser(description="Analyze LLaDA diffusion steps and sparsity logs.")
    parser.add_argument("--dir", type=str, required=True, help="Path to the experiment directory (SCRIPT_DIR)")
    ...
    # 1. Automatically find files
    try:
        npz_file = find_file(DIR, "*_mask_snapshots.npz")
        sparsity_json = find_file(DIR, "*_sparsity_per_step.json")
        sparsity_csv = find_file(DIR, "*_summary.csv")
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return

    # 2. Load and process data
    print(f"Analyzing directory: {DIR}")
    snaps = load_snapshots(npz_file)
    df_sparsity_summary = pd.read_csv(sparsity_csv)
    df_sparsity_summary['layer_num'] = df_sparsity_summary['Layer'].apply(layer_num_from_name)
    ...
    if not stab.empty and 'layer_num' in stab:
        ...
        plt.xlabel('Step');
        plt.ylabel('Mean per‑token Jaccard')
        plt.title(f'Layer {mid_layer_num}: stability vs interval');
        plt.legend();
        plt.grid(True, alpha=0.3)
        plt.tight_layout();
        plt.savefig(out / f"layer{mid_layer_num}_stability_timeseries.png", dpi=300);
        plt.close()
    ...
    data_json = load_sparsity_json(sparsity_json)
    df = sparsity_json_to_df(data_json)
