...
if __name__ == "__main__":
    root = (
        ...
        / "results"
        / "gate_relufication_variants_fineweb_200m"
    )
    standard_relufication_dir = Path(__file__).parent.parent.parent / 'results' / 'gate_relufication_fineweb_200m'
    ...
    standard_relufication_files = list(f for f in standard_relufication_dir.rglob("**/results*.json"))
    standard_relufication_files = [f for f in standard_relufication_files if 'hard' in str(f)]
    standard_relufication_df = read_standard_harness_results(standard_relufication_files, parser, config_keys)

    df = pd.concat([baseline_df, standard_relufication_df, df], ignore_index=True)
