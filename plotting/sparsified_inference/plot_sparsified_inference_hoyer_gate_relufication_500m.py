...
if __name__ == "__main__":
    ...
    results_dir = root / "sparsified_evaluation_hoyer" / "llama3-1b-hoyer-gate-relufication"
    ...
    for setup in setups:
        ...
        try:
            ...
            continue
        df["sparsification_loss"] = f"ReLUfication ({sparsification_loss.replace('cosine', 'cos').replace('inverse', 'inv')})"
        df = df[df['sparsification_loss'] == 'ReLUfication (hard)']
        df['sparsification_loss'] = 'ReLUfication (200M)'
        df["model"] = model
    ...
    for setup in setups:
        ...
        df = read_sparsified_harness_results(sparsity_stat_files)
        df["sparsification_loss"] = "Baseline (200M)"
        df["model"] = model
        all_data_abs.append(df)
    ...
    # Load data for 500M models
    results_dir = root / "sparsified_evaluation_500m" / "llama3-1b"
    setups = sorted(
        list(
            set(
                [
                    child
                    for child in (root / "sparsified_evaluation_500m").glob("*/*")
                    if child.is_dir()
                ]
            )
        )
    ...
    for setup in setups:
        ...
        if 'baseline' in sparsification_module:
            sparsification_loss = "Baseline (500M)"
        elif 'relufication' in sparsification_module:
            sparsification_loss = "ReLUfication (500M)"
        else:
            raise ValueError(
                f"Unexpected sparsification module: {sparsification_module}"
            )
        sparsity_stat_files = list(setup.rglob("sparsity_stats.json"))
        sparsity_stat_files = [f for f in sparsity_stat_files if "topp" in str(f)]
        df = read_sparsified_harness_results(sparsity_stat_files)
