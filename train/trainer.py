...
class SparsityEnforcementTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        # elif loss_type == "none":
        #     self.sparsification_loss = lambda tensor, attn_mask: 0
        # else:
    ...
    def compute_loss(
        ...
        # loss_dict = {
        #     'loss_cross_entropy': loss.mean().item()
        # }
        # if self.loss_weight != 0:
        ...
        #     ]
        #     sparsification_loss = torch.mean(torch.tensor(sparsification_losses))
        #     loss_dict["loss_sparsity"] = sparsification_loss.item()
    ...
    def prediction_step(
        ...
        #                 )
        #                 num_sparse_neurons = (
        #                     (sparse_neurons_mask * attn_mask.to(sparse_neurons_mask.device, non_blocking=True).unsqueeze(-1))
        #                     .sum(-1)
        #                     .sum(-1)
        #                 )
