...
for params in "${model_setups[@]}"; do
    ...
    for sparsity_setup in "${sparsity_setups[@]}"; do
        ...
        for task in arc_easy arc_challenge boolq hellaswag lambada piqa sciq triviaqa winogrande; do
            ...
            sbatch -A plgdynamic2-gpu-a100 -p plgrid-gpu-a100 --job-name=${eval_job_name} --output=${model_logs_dir}/${model_eval_dir}/${task}_${run_name}.out slurm_scripts/harness/evaluate_sparsified.sh ${model_path} ${task} ${eval_dir} ${run_name} ${sparsification_config} ${sparsification_rule} ${sparsification_th}
        done
    done
done