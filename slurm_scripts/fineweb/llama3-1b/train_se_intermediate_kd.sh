...
num_epochs=1
batch_size=1 # Max batch size for Athena jobs
...
python train/main.py \
    ...
    --run_name llama3-1b-fineweb-s${num_samples}-intermediate-sparsification-kd-w-${kd_loss_weight}-temp-${kd_temperature}-${sparsity_loss}-w-${sparsity_weight}-nepochs-${num_epochs}-bs-${actual_batch_size}-lr-${lr} \
    ...
    --wandb_tags fineweb-s${num_samples}-intermediate-sparsification-kd
