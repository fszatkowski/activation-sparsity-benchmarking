...
# Evaluation setup
eval_dir=results/gate_relufication_fineweb_200m
...
hyperparams=(
    "5e-6 hard llama3-1b-fineweb-200m-gate-relufication-hard"
    "5e-6 soft llama3-1b-fineweb-200m-gate-relufication-soft"
    "5e-6 soft_cosine llama3-1b-fineweb-200m-gate-relufication-soft-cosine"
    "5e-6 soft_inverse_cosine llama3-1b-fineweb-200m-gate-relufication-soft-inverse-cosine"
)
...
for params in "${hyperparams[@]}"; do
    ...
    output_dir=output_models/FineWeb/Llama-3.2-1B-gate-relufication/s${num_samples}-${relufication_mode}-bs-${actual_batch_size}-lr-${lr}

    # Create a unique job name based on the parameters
    job_name="llama3-1b-fineweb-s${num_samples}-lr-${lr}-gate-relufication-${relufication_mode}-train"

    # Submit the job to slurm and get it's job id
    echo "Submitting job: ${job_name}"
    job_id=$(sbatch -A plgdynamic2-gpu-a100 -p plgrid-gpu-a100 --job-name=${job_name} slurm_scripts/fineweb/llama3-1b/train_relufication_gate.sh ${lr} ${num_samples} ${gacc} ${relufication_mode} ${output_dir} | awk '{print $4}')
