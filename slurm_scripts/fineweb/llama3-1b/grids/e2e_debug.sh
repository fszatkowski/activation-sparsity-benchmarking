...
for params in "${hyperparams[@]}"; do
    ...
    output_dir=debug/output_models/FineWeb/Llama-3.2-1B/baseline-s${num_samples}-bs-${actual_batch_size}-lr-${lr}
    ...
    for task in arc_easy winogrande; do
        # Create a unique job name based on the parameters
