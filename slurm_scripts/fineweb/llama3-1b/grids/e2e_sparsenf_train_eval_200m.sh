...
# Evaluation setup
eval_dir=results/continued_ft_fineweb_200m
...
hyperparams=(
    "5e-6 hoyer 0.0000003 llama3-1b-fineweb-200m-sparsity-loss-hoyer-w0.0000003-lr-5e-6"
    "5e-6 hoyer 0.0000005 llama3-1b-fineweb-200m-sparsity-loss-hoyer-w0.0000005-lr-5e-6"
    "5e-6 l1 0.0000005 llama3-1b-fineweb-200m-sparsity-loss-l1-w0.0000005-lr-5e-6"
    "5e-6 l1 0.0000003 llama3-1b-fineweb-200m-sparsity-loss-l1-w0.0000003-lr-5e-6"
)
...
for params in "${hyperparams[@]}"; do
    ...
    output_dir=output_models/FineWeb/Llama-3.2-1B/s${num_samples}-sparsity-loss-${sparsity_loss}-bs-${actual_batch_size}-lr-${lr}-sw-${sparsity_weight}
    ...
    job_id=$(sbatch -A plgdynamic2-gpu-a100 -p plgrid-gpu-a100 --job-name=${job_name} slurm_scripts/fineweb/llama3-1b/train_se.sh ${lr} ${num_samples} ${gacc} ${sparsity_loss} ${sparsity_weight} ${output_dir} | awk '{print $4}')
